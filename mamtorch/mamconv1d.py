...
class MAMConv1dFunction(torch.autograd.Function):
    ...
    def forward(ctx, X, W, stride):
        ...
        ctx.save_for_backward(X, W, argmax, argmin, stride)
        return Y, argmax, argmin

    @staticmethod
    def backward(ctx, Y_grad, argmax_grad, argmin_grad):
        X, W, argmax, argmin, stride = ctx.saved_tensors
        X_grad, W_grad = mamtorchkernel.mamconv1d_backward(X, W, Y_grad, argmax, argmin, stride)
...
class MAMConv1d(torch.nn.Module):
    ...
    def update_selection_count(self):
        ...
        num_batches, num_channels, num_elements = self.argmax.shape
        filter_indices = torch.arange(num_channels).repeat_interleaved(num_elements).repeat(num_batches).to(self.weight.device)
        ...
        num_batches, num_channels, num_elements = self.argmin.shape
        filter_indices = torch.arange(num_channels).repeat_interleaved(num_elements).repeat(num_batches).to(self.weight.device)
