// original code https://github.com/siboehm/SGEMM_CUDA/tree/master
...
__global__ void __launch_bounds__(NUM_THREADS) sgemmWarptiling(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C)
{
    ...
    Cpt += (cRow * BM + warpRow * WM) * N + cCol * BN + warpCol * WN;
    ...
    {
        wt::loadFromGmem<BM, BN, BK, rowStrideA, rowStrideB>(N, K, Apt, Bpt, As, Bs, innerRowA, innerColA, innerRowB, innerColB);
        __syncthreads();
        ...
        Apt += BK;     // move BK columns to right
        Bpt += BK * N; // move BK rows down
        __syncthreads();
    }
    ...
    for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx)
    {
        for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx)
        {
            ...
            float *C_interim = Cpt + (wSubRowIdx * WSUBM) * N + wSubColIdx * WSUBN;
