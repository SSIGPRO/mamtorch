...
__global__ void fullyconnected_cuda_kernel_acc4(
    const float * __restrict__ A,
    const float * __restrict__ BT,
    float * __restrict__ C,
    int * __restrict__ Cargmax,
...
__global__ void fullyconnected_cuda_kernel_acc128(
    const float * __restrict__ A,
    const float * __restrict__ BT,
    float * __restrict__ C,
    int * __restrict__ Cargmax,
...
__global__ void fullyconnected_cuda_kernel_acc256(
    const float * __restrict__ A,
    const float * __restrict__ BT,
    float * __restrict__ C,
    int * __restrict__ Cargmax,
...
{   
    ...
    switch(accblock_size)
    {
        ...
        case 64:
            ...
            break;
        case 128:
            fullyconnected_cuda_kernel_acc128<<<blocks, threads>>>(
                A_padded.data_ptr<float>(),
                BT_padded.data_ptr<float>(),
                C_padded.data_ptr<float>(),
                Cargmax_padded.data_ptr<int>(),
            ...
            break;
        case 256:
            fullyconnected_cuda_kernel_acc256<<<blocks, threads>>>(
                A_padded.data_ptr<float>(),
                BT_padded.data_ptr<float>(),
                C_padded.data_ptr<float>(),
                Cargmax_padded.data_ptr<int>(),
            ...
            break;
        default:
            throw std::invalid_argument("Invalid size for accumulation blocks (forward)");
    }
    ...
    auto C = CTcm;
    auto Cargmax = torch::clamp(CargmaxTcm, 0, K/accblock_size-1);
    auto Cargmin = torch::clamp(CargminTcm, 0, K/accblock_size-1);
