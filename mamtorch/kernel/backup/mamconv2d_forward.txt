...
__global__ void mamconv2d_forward_cuda_kernel(
...
{    
    ...
    const int num_blocks = Nf*Mf*C/BSK;
    for(int bk = 0; bk < num_blocks; ++bk)
    {        
        // put data in the shared input blocks
        for(int l = 0; l < LPT; ++l)
        {
            const int filter_index = bk/(C/BSK);
            const int filter_i = filter_index%Mf;
            const int filter_j = filter_index/Mf;
            ...
            const int Xblock_j = bk%(C/BSK)*BSK + block_j;
    ...
    const int i_filter_out = bz%Mo;
    const int j_filter_out = bz/Mo;
    ...
    for(int wi = 0; wi < WPT; ++wi)
    {
        ...
        for(int wj = 0; wj < WPT; ++wj)
        {
            ...
            Y[j_filter_out*Mo*B*F + i_filter_out*B*F + j_out*B + i_out] = accmax[wi][wj];
            Yargmax[j_filter_out*Mo*B*F + i_filter_out*B*F + j_out*B + i_out] = argmax[wi][wj];
            Yargmin[j_filter_out*Mo*B*F + i_filter_out*B*F + j_out*B + i_out] = argmin[wi][wj];
        }
    }
}
...
{    
    ...
    const auto Mo = Y.size(1);
    const auto No = Y.size(0);
    ...
    if(Fpad || Cpad)
    { 
        Wpad = torch::pad(W, torch::IntList{0, Fpad, 0, Cpad},
                          "replicate").squeeze();
    }
    
    if(Bpad || Cpad)
    { 
        Xpad = torch::pad(X, torch::IntList{0, Bpad, 0, Cpad},
                          "replicate").squeeze();
    }
    ...
    if(Fpad || Bpad)
    {
        Ypad = torch::empty({No, Mo, F+Fpad, B+Bpad}, Y.options());
        Yargmax_pad = torch::empty({No, Mo, F+Fpad, B+Bpad}, Yargmax.options());
        Yargmin_pad = torch::empty({No, Mo, F+Fpad, B+Bpad}, Yargmin.options());
    }
    ...
    AT_DISPATCH_FLOATING_TYPES(X.scalar_type(),
                               "mamconv2d_forward_cuda_kernel", ([&]{
    mamconv2d_forward_cuda_kernel<scalar_t><<<blocks, threads>>>(
        Xpad.data_ptr<scalar_t>(),
        Wpad.data_ptr<scalar_t>(),
        Ypad.data_ptr<scalar_t>(),
        Yargmax_pad.data_ptr<int>(),
        Yargmin_pad.data_ptr<int>(),
        (B+Bpad), (C+Cpad), N, M, (F+Fpad), Nf, Mf, Mo, hstride, wstride, Cpad);
    }));
    */
    
    mamconv2d_forward_cuda_kernel<float><<<blocks, threads>>>(
        Xpad.data_ptr<float>(),
        Wpad.data_ptr<float>(),
        Ypad.data_ptr<float>(),
        Yargmax_pad.data_ptr<int>(),
    ...
    if(Fpad || Bpad)
    {
        Y.copy_(Ypad.slice(2, 0, F).slice(3, 0, B));
        Yargmax.copy_(Yargmax_pad.slice(2, 0, F).slice(3, 0, B));
        Yargmin.copy_(Yargmin_pad.slice(2, 0, F).slice(3, 0, B));
    }
}